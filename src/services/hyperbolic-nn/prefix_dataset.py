...
parser.add_argument(
    ...
    type=str,
    help="Root path",
    default="/path/to/a/folder/where/prefix/datasets/will/be/created/",
)
...
def gen_data(data_size):
    ...
    for _ in range(int(data_size / 2)):
        ...
        sent1_prefix = [
            randint(1, vocab_size) for _ in range(sent2_len)
        ]  # Shorter sentence
        sent1 = sent1_prefix + [
            randint(1, vocab_size) for _ in range(sent1_len - sent2_len)
        ]
